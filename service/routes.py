...
def get_product(product_id):
    """
    endpoint to get a single Product based on product_id
    """
    app.logger.info(f"Request to retrieve a product with id {product_id}")
    if not product_id.isdigit():
        abort(status.HTTP_400_BAD_REQUEST, f"Request id {product_id} was not valid")
...
def update_product(product_id):
    """
    This endpoint updates a single Product based on product_id with the request body
    """
